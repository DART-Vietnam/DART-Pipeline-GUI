...
def run_subproc(
    ...
    st_session_state["log"] += f"Started subprocess with pid {subproc.pid}\n"
    st_console.code(st_session_state["log"], language="bash", height=300)

    for line in subproc.stdout:  # pyright: ignore[reportOptionalIterable]
        st_session_state["log"] += line
        st_console.code(st_session_state["log"], language="bash", height=300)

    sigterm = st.button("Terminate ⛔", key=f"sigterm_{subproc.pid}")
    if sigterm:
        subproc.terminate()
